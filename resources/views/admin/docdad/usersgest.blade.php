    ...
    <li class='sidenav__list-item sub-menu '>
     ...
     <a href="" class="dropdown-toggle"  data-toggle="dropdown" style="color:white; font-weight:bold"> AJOUTER UN DOCUMENT </a>
      <ul>
        ...
        <li class="sidenav__list-item active" style="color:white; font-weight:bold"> <a href="{{ route('admin.userslist') }}"  style="color:white; font-weight:bold">GESTION DES UTILISATEURS</a></li>
...
@endif
     ...
     <h3 style=" display: flex; align-items: center; justify-content: center; "> USERS MANAGEMENT </h3>
      ...
      <div class="carda">
        ...
        <table>
          ...
          <tbody>
            ...
            <tr class="panel panel-info" data-id="{{ $user->id }}">
              
              <td data-label="#">{{ $user->id }}</td>
            <td data-label="Prénom">{{ $user->firstname }}</td>
            
            <td data-label="Nom">{{ $user->lastname }}</td>
            <td data-label="Accreditation">{{ $user->visibility()->first()->name }}</td>
            <!--<td><input type="checkbox" name="checkbox[]" value="" ></td>-->
            <td data-label="Status" class="favori">
              @if ($user->valid==true)
               <a  class="btn btn-success">Activé </a>
              @else
                <a class="btn btn-danger ">Désactivé </a>  
              @endif
...
$('.favori').click(function(){ 
    ...
    var user_id =$(this).parents(".panel").data('id');
    $.ajax({
            type:'POST',
          ...
          if(data.success==false){
            ...
            //var numb=idbis-1;
            //idbis=idbis+1;
            cObj.parents(".panel").find(".favori").html(`<a  class="btn btn-success favori">Activé </a>` );
            //idbis=idbis+1;
            //$(cObj).removeClass("like-post");
          }else if(data.success==true){
            //idbis=idbis+1;
            cObj.parents(".panel").find(".favori").html(` <a  class="btn btn-danger favori">Désactivé </a>` );
            //idbis=idbis-1;
            //$('#'+cObjId+'-bs3').text(parseInt(c)+1);
